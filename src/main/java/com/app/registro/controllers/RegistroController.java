...
//  ****************************	REGISTRO 	***********************************  //
	...
	// CREAR REGISTRO - TESTING
	@PostMapping("/registro/crear-testing/")
	@ResponseStatus(code = HttpStatus.CREATED)
	public String crearRegistroTesting(@RequestBody @Validated Registro registro) {
		if (!(Boolean) cbFactory.create("registro")
				.run(() -> uClient.preguntarUsuarioExiste(registro.getUsername(), registro.getEmail(),
								registro.getCellPhone()),
						e -> preguntarUsuarioExiste2(registro.getUsername(), registro.getEmail(),
								registro.getCellPhone(), e))) {
			rService.crearNuevoUsuarioTesting(registro);
			Registro r = rRepository.findByUsername(registro.getUsername());
			Usuario usuario = rService.crearUsuarios(r);
			r.setCodigo("0");
			if (cbFactory.create("registro").run(
					() -> uClient.crearUsuarios(usuario, r.getPassword(), r.getRoles()),
					e -> crearUsuarios2(e))) {
				rRepository.delete(r);
				return "Usuario Creado Exitosamente";
			}
			throw new ResponseStatusException(HttpStatus.CONFLICT, "Error en la creacion");
		}
		throw new ResponseStatusException(HttpStatus.CONFLICT, "El usuario ya existe");
	}
